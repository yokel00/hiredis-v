...
static int32_t _redisClusterAuth(redisClusterContext *cc, redisContext *c)
{
	if (cc->password[0] == '\0')
		return REDIS_OK;
...
static void __redisClusterSetError(redisClusterContext *cc, int type, const char *str) {
	...
	cc->err = type;
	if (str != NULL) {
		strncpy(cc->errstr, str, sizeof(cc->errstr));
	}
	else {
	...
	const char *ip, int port)
{
	...
	if (_redisClusterAuth(cc, c) != REDIS_OK) {
		goto error;
	}
...
int redisClusterSetOptionPassword(redisClusterContext* cc, const char *password) {
	if (password) {
		strncpy(cc->password, password, sizeof(cc->password));
	}
	else {
		cc->password[0] = '\0';
	}
...
static void __redisClusterAsyncSetError(redisClusterAsyncContext *acc,
	...
	acc->err = type;
	if (str != NULL) {
		strncpy(acc->errstr, str, sizeof(acc->errstr));
	}
	else {
		/* Only REDIS_ERR_IO may lack a description! */
		assert(type == REDIS_ERR_IO);
	...
	cluster_node *node)
{
	...
	if (acc->cc->password[0]) {
		redisAsyncCommand(ac, NULL, NULL, "AUTH %s", acc->cc->password);
		printf("auth after connect.\n");
	}

	// _redisClusterAuth(acc->cc, &ac->c);
...
static void redisClusterAsyncCallback(redisAsyncContext *ac, void *r, void *privdata) {
	...
	if (cad != NULL)
	{
		// if cmd is subscribe/psubscribe, don't free!
		if(cad->command->type != CMD_REQ_REDIS_SUBSCRIBE
			&& cad->command->type != CMD_REQ_REDIS_PSUBSCRIBE)
